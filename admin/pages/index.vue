        ...
        >
          <div class="history-box">
            ...
            <div class="a-row">
              ...
              <span class="a-color-tertiary a-size-small asin-reviews">{{ product.rating }}</span>
            </div>
            <!-- Product price -->
            <div class="a-row">
...
export default {
  ...
  async asyncData({ $axios }) {
    try {
      let response = await $axios.$get("https://heroku-app-backend-c139fe7f175d.herokuapp.com/api/products");
    ...
    async onDeleteProduct(id, index) {
      try {
        let response = await this.$axios.$delete(
          `https://heroku-app-backend-c139fe7f175d.herokuapp.com/api/products/${id}`
        );
        if (response.status) {
          // remove
